...
export function PatientView({ dashboard, ready, prediction }: PatientViewProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/70 dark:border-slate-600 border-l-4 border-l-slate-900 px-8 py-7">
        <h1 className="text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-100 mb-3">Welcome, {patientFirstName(patient)}.</h1>
      ...
      <section className="grid grid-cols-2 gap-4">
        <ContributorCard
          ...
          icon={<ShieldCheck size={18} className="text-emerald-800 dark:text-emerald-300" />}
          items={d.strengths.slice(0, 3)}
        />
        <ContributorCard
          tone="action"
          title="Action Items"
          icon={<AlertTriangle size={18} className="text-red-900 dark:text-red-300" />}
        ...
        <div className="grid grid-cols-[1.4fr_1fr] gap-8">
          <div>
            ...
            <button
              ...
              className="mt-6 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-sm font-semibold px-6 py-2.5 rounded-lg hover:bg-slate-700 dark:hover:bg-slate-300 disabled:opacity-50 transition-colors"
            >
              {dashboard.isClinicianImpactSubmitting ? "Calculating…" : "Calculate"}
            </button>
          </div>

          <div className="rounded-xl border border-slate-200/70 dark:border-slate-700/70 bg-slate-50/60 dark:bg-slate-800/60 p-6 flex flex-col items-center text-center">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-100 mb-2">Estimated Treatment Response</p>
            <ResponseBadge label={responseBadge.label} classes={responseBadge.classes} />
            <p className="text-5xl font-bold text-slate-900 dark:text-slate-100 tracking-tight mt-3">{pct(displayedResponse)}</p>
            ...
            <div className="w-full mt-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 mb-1 text-center">Response Over Time</p>
              <TimeToEventChart
