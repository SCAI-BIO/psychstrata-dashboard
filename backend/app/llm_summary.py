...
def fetch_llm_summary(prompt: str) -> str:
    ...
    try:
        ...
        raise LLMServiceError(
            f"Prediction explanation unavailable: the language model service could not be reached ({reason})."
        ) from exc
    except TimeoutError:
        raise LLMServiceError("Prediction explanation unavailable: the language model service timed out.")
