...
def generate_synthetic_dataset(
    ...
    quetiapine_mg = np.clip(
        rng.choice([0, 50, 100, 150, 200, 300], size=n, p=[0.5, 0.15, 0.12, 0.1, 0.08, 0.05]),
        0,
        300,
    )
    lithium_mg = np.clip(
        rng.choice([0, 300, 600, 900, 1200], size=n, p=[0.7, 0.08, 0.1, 0.07, 0.05]),
        0,
        1200,
    )

    early_improvement = rng.choice([0, 1], size=n, p=[0.6, 0.4])
    sleep_severity = rng.choice([0, 1, 2], size=n, p=[0.3, 0.45, 0.25])
    substance_use = rng.choice([0, 1, 2], size=n, p=[0.7, 0.2, 0.1])
    ...
    phq_eff = 0.12 * phq9
    duration_eff = 0.06 * (duration_months / 6.0)
